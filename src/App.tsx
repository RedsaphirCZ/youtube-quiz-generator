...
export default function App() {
  const [mode, setMode] = useState<'home' | 'classic' | 'pictures' | 'languages' | 'maps' | 'country-assets'>('home');
  ...
  return <Suspense fallback={<main role="status" className="min-h-screen flex items-center justify-center">Loading studio…</main>}>
    {mode === 'classic' ? <ClassicStudio onExit={exit} /> : mode === 'pictures' ? <PictureQuizStudio onExit={exit} /> : mode === 'languages' ? <LanguageQuizStudio onExit={exit} /> : mode === 'maps' ? <MapQuizStudio onExit={exit} /> : mode === 'country-assets' ? <CountryAssetStudio onExit={exit} /> :
