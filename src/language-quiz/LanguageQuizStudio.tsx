...
import { embedPhraseFlags, flagPath, generatePhrasePrompt, loadPhraseLibrary, parsePhraseDeck, phraseDeckHTML, phraseProfiles, savePhraseDeck } from './phraseCards';
import type { PhraseConfig, PhraseDeck } from './phraseCards';
  ...
  return <main className="min-h-screen bg-[#f3f0f8] px-4 py-6 text-slate-900 sm:py-10">
    <section className="mx-auto max-w-5xl overflow-hidden rounded-3xl border border-violet-200 bg-white shadow-lg">
      <header className="border-t-[6px] border-violet-700 bg-violet-50 p-5 sm:p-8">
        ...
        <p className="mt-3 text-sm text-slate-600">Choose a researched learning quiz or translate one phrase into all 24 languages with IPA and printable cards.</p>
      </header>
      <div className="p-5 sm:p-8">
        ...
        {view === 'home' && <>
          <div className="grid gap-5 lg:grid-cols-2">
            ...
            <section className="rounded-2xl border-2 border-fuchsia-200 bg-fuchsia-50 p-5 sm:p-6">
              ...
              <p className="mt-2 text-sm text-slate-600">Give Gemini one phrase. It returns all 24 translations in native scripts, full-phrase IPA, named varieties and printable flag cards.</p>
        ...
        {view === 'phraseReview' && phraseDeck && <section className="space-y-5"><div><p className="text-xs font-black uppercase tracking-widest text-fuchsia-700">Unreviewed phrase deck · {phraseDeck.cards.length} languages</p><h2 className="mt-2 text-2xl font-black" dir="auto">{phraseDeck.title}</h2><p className="mt-2 text-sm text-slate-600"><strong>Source:</strong> <span dir="auto">{phraseDeck.sourcePhrase}</span>{phraseDeck.context ? ` · ${phraseDeck.context}` : ''}. Check translations and IPA with a fluent speaker or trusted reference before publishing.</p></div>
          ...
          <div className="grid gap-4 md:grid-cols-2">{phraseDeck.cards.map((card, i) => <article key={card.language} className="overflow-hidden rounded-2xl border border-fuchsia-100 bg-white shadow-sm"><div className="grid min-h-52 grid-cols-2"><div className="flex items-center justify-center border-r border-dashed border-fuchsia-200 bg-fuchsia-50 p-4 text-center"><p className="text-xl font-bold leading-relaxed" lang={card.language} dir={languageDirection(card.language)}>{card.phrase}</p></div><div className="flex flex-col items-center justify-center p-4 text-center"><p className="text-lg font-bold leading-relaxed" lang={card.language} dir={languageDirection(card.language)}>{card.phrase}</p><div className="mt-3 flex flex-wrap justify-center gap-1">{phraseProfiles[card.language].flags.map(country => <img key={country} className="h-5 w-7 object-contain" src={flagPath(country)} alt={`${country.toUpperCase()} flag`} />)}</div><h3 className="mt-2 font-black">{languageName(card.language)}</h3><p className="mt-1 text-sm" dir="ltr">{card.ipa}</p><p className="mt-1 text-[10px] text-slate-500">{card.variety}</p></div></div><p className="border-t border-fuchsia-100 px-4 py-2 text-xs font-bold text-fuchsia-700">{String(i + 1).padStart(2, '0')} / {phraseDeck.cards.length} · front / back preview</p></article>)}</div>
        </section>}
